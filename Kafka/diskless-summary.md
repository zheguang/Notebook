...
| **KIP-1176** | **Partial** - Only follower replication path (producer→leader and consumer traffic unchanged) | **Moderate** - 43% overall cost reduction documented | **Low latency** - Maintains single-digit ms for acks=1, near-traditional for acks=-1 | **Limited** - Still broker-centric, no cloud-native elasticity benefits | **Weak** - AZ failure creates recovery challenges, no hot standby for active segments | **Low-Medium** - Incremental changes to existing code, but complexity may grow | Under discussion, availability concerns raised |
...
- Claims 43% cost reduction while maintaining low latency performance
